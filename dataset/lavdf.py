...
class Lavdf(Dataset):
    ...
    def __init__(self, subset: str, root: str = "data", frame_padding: int = 512,
        ...
        label_dir = os.path.join("/kaggle/working/", "label")
    ...
    def get_label(self, meta: Metadata) -> Tensor:
        ...
        path = os.path.join("/kaggle/working/", "label", file_name)
    ...
    def get_label_with_match_scores(self, meta: Metadata) -> Tuple[Tensor, Tensor, Tensor]:
        Path(os.path.join("/kaggle/working/", "label")).mkdir(parents=True, exist_ok=True)
        Path(os.path.join("/kaggle/working/", "match_scores")).mkdir(parents=True, exist_ok=True)

        boundary_map_file_name = meta.file.split("/")[-1].split(".")[0] + ".npy"
        boundary_map_file_path = os.path.join("/kaggle/working/", "label", boundary_map_file_name)

        match_scores_file_name = meta.file.split("/")[-1].split(".")[0] + ".npz"
        match_scores_file_path = os.path.join("/kaggle/working/", "match_scores", match_scores_file_name)
